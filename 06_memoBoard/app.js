...
board.addEventListener('mousedown', (event) => {
  ...
  offsetXStart = event.offsetX;
  offsetYStart = event.offsetY;

  // if another memo it not currently being moved around.
  if (!movingMemo) {
...
board.addEventListener('mouseup', (event) => {
  mouseClicked = false;
  offsetXEnd = event.offsetX;
  offsetYEnd = event.offsetY;

  let width = offsetXEnd - offsetXStart;
  let height = offsetYEnd - offsetYStart;

  // Momo has to be larger than 50px * 50px and user should not be currently repositioning another memo.
...
board.addEventListener('mousemove', (event) => {
  if (mouseClicked && !movingMemo && !resizingMemo) {
    offsetXCurrent = event.offsetX - offsetXStart;
    offsetYCurrent = event.offsetY - offsetYStart;
...
class Memo {
  ...
  mouseDownMove(event) {
    ...
    // determine where the grab cursor is to position the memo relative the offset and mouse position.
    this.movingXDist = event.clientX - this.position.left;
    this.movingYDist = event.clientY - this.position.top;
  }
  ...
  moveMemo(event) {
    ...
    this.div.style.top = `${event.clientY - this.movingYDist}px`;
    this.div.style.left = `${event.clientX - this.movingXDist}px`;
  }

  resizeMemo(event) {
    let height = event.clientY - this.position.top;
    let width = event.clientX - this.position.left;
...
localStorageMemos.forEach(memo => {
  let storedMemo = new Memo(
    memo.id,
    {
      left: memo.position.left,
      top: memo.position.top
    },
    {
...
function updateLocalStorage() {
  if (localStorage.getItem('memos') != JSON.stringify(memoList)) {
    console.log('Local storgae updated');
...
window.addEventListener('mousemove', (event) => {
  for (let i = 0; i < memoList.length; i++) {
    if (memoList[i].moving) {
      memoList[i].moveMemo(event);
    }

    if (memoList[i].resizing) {
      memoList[i].resizeMemo(event);
    }
  }
});
